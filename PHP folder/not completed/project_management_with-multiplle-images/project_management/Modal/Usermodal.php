...
//            $insert = $this->db->prepare("INSERT INTO image(images,model_type,model_id)VALUES('$imgPath','project','$idValue')");
//            $insert->execute();
            $insertImg->execute();
        }
        header('location:/');
...
//        var_dump($n);
    ...
    public function taskDetails($taskDetails,$img){
        echo "<pre>";
//        var_dump($_FILES['taskImage']['tmp_name']);echo "</pre>";

        $taskId = $taskDetails['getTaskid'];
        $taskName = $taskDetails['taskName'];
        $taskDescription = $taskDetails['taskDescription'];
        $insertTask = $this->db->query("INSERT INTO tasks(task_name,task_description,project_id)VALUES('$taskName','$taskDescription','$taskId')");


        $inn = $this->db->query("SELECT id FROM tasks ORDER BY id DESC LIMIT 1;");
        $id = $inn->fetch();
        $ids = $id['id'];
//        var_dump($ids);
        foreach ($_FILES['taskImage']["tmp_name"] as $key =>$value){
           ...
           $file_name = $_FILES['taskImage']['name'][$key];
            ...
            $imgPath = "images/".$file_name;
            move_uploaded_file($file_tmpname,$imgPath);
            $insertImg = $this->db->prepare("INSERT INTO images(images,model_type,model_id)VALUES ('$imgPath','task','$ids')");
//        echo $ids;
//            $insert = $this->db->query("INSERT INTO image(model_type,model_id)VALUES('task','$ids')");
            $insertImg->execute();
//            $insert->execute();
//echo 'inserted';
            header('location:/');
